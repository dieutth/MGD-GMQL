    ...
    val reduced = RefJoinedExp
      ...
      .mapValues {
        x =>
          ...
          val (expIds, counts) = (allExpIdsWithCount ++ (x._1._2 zip x._3) groupBy(x=>x._1) mapValues(_.map(_._2).sum)).toArray.unzip
          val refId = x._1._1
          val ids = for (expId <- expIds)
                       yield Hashing.md5().newHasher().putLong(refId).putLong(expId).hash().asLong

          val features: Array[Array[Double]] = for (count <- counts)
                                                  yield x._2 :+ count.toDouble
