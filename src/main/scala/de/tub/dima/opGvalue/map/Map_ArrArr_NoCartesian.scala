...
object Map_ArrArr_NoCartesian {
  ...
  def apply(
             sc: SparkContext,
             ref: RDD[((String, Long, Long, Short), (Array[Long],Array[Array[GValue]]))],
             exp: RDD[((String, Long, Long, Short), (Array[Long],Array[Array[GValue]]))],
             bin: Int
           ): RDD[((String, Long, Long, Short), (Array[Long], Array[Array[GValue]]))] = {
      ...
      .mapValues {
        x =>
          ...
          val features = for (refFeature <- x._2;
                              expFeature <- counts)
            yield refFeature :+ GDouble(expFeature.toDouble)
          (ids, features)
      }
