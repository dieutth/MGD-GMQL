...
object GenometricJoin {
  ...
  def apply(executor : MGD_GMQLSparkExecutor, metajoinCondition : OptionalMetaJoinOperator, joinCondition : List[JoinQuadruple], regionBuilder : RegionBuilder, leftDataset : RegionOperator, rightDataset : RegionOperator, sc : SparkContext) : RDD[GRECORD] = {
    // load datasets
    val ref : RDD[GRECORD] =
