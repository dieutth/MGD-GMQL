...
object GenometricJoin2 {
  ...
  def apply(executor : MGD_GMQLSparkExecutor, metajoinCondition : OptionalMetaJoinOperator, joinCondition : List[JoinQuadruple], regionBuilder : RegionBuilder, leftDataset : RegionOperator, rightDataset : RegionOperator, BINNING_PARAMETER:Long, MAXIMUM_DISTANCE:Long, sc : SparkContext) : RDD[GRECORD] = {
    // load datasets
    val ref : RDD[GRECORD] =
