...
import it.polimi.genomics.core.{GDouble, GMQLSchemaFormat, GRecordKey, GValue}
...
object Benchmark{
    def apply(conf: SparkConf, refFilePath: String, expFilePath: String,
              outputPath: String) = new Benchmark(conf, refFilePath, expFilePath, outputPath)
}


class Benchmark(conf: SparkConf, refFilePath: String, expFilePath: String, outputPath: String){
  ...
  def benchmarkCurrentSystem_Map(loop: Int): Unit = {
    ...
    server = new GmqlServer(new GMQLSparkExecutor(sc=sc,outputFormat = GMQLSchemaFormat.TAB))
